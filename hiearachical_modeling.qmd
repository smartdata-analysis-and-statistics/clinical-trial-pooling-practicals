...
- It enrolls patients with **higher baseline severity** (mean = 35 vs. 25 in other trials)
- It also exhibits **lower overall outcome levels**, due to an added **negative intercept shift** (−15) applied equally to both treatment arms

This setup reflects a situation where one trial includes a more severely affected population and reports systematically lower outcome scores, even though the treatment effect remains consistent across all trials.
...
# Simulate 5 trials with 100 patients each (50 per arm)
df <- simulate_trials(
  baseline_means = c(25,25,25,25,35), # Different mean baselines across trials
...
Note that all trial-specific intercepts are no longer similar, and now range from approximately `r round(min(intercepts),2)` to `r round(max(intercepts),2)`. 
