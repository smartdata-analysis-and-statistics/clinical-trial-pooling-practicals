...
- `trt`: Estimates the average treatment effect across trials
- `trt:baseline`: Interaction between baseline and treatment
- `(0 + trt | trial)`: Allows the treatment effect to vary randomly by trial
...
We can now visualize the treatment effect as a function of the baseline coviarate:
...
ggplot(te_df, aes(x = baseline, y = treatment_effect)) +
  ...
  labs(
    title = "Estimated Treatment Effect by Baseline Severity",
    x = "Baseline",
