...
We can estimate the pooled treatment effect by modelling the observed outcome as a function of treatment allocation and baseline severity:

```{r}
#| message: false
((fit <- glm(outcome ~ trta + baseline, data = df)))
```
