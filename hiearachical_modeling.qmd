...
# Extract coefficient estimates and 95% CIs
ci <- confint(fit_hte_3b)

gamma_w  <- coef(fit_hte_3b)["trt:cbase"]
ci_w     <- ci["trt:cbase", ]

gamma_a  <- coef(fit_hte_3b)["trt:xk_mean"]
ci_a     <- ci["trt:xk_mean", ]

# Build caption text with 2 decimal places
caption_text <- glue::glue(
  "Within-trial interaction: {formatC(gamma_w, format = 'f', digits = 2)} ",
  "({formatC(ci_w[1], format = 'f', digits = 2)}; {formatC(ci_w[2], format = 'f', digits = 2)}); ",
  "Across-trial interaction: {formatC(gamma_a, format = 'f', digits = 2)} ",
  "({formatC(ci_a[1], format = 'f', digits = 2)}; {formatC(ci_a[2], format = 'f', digits = 2)})"
)
...
# Calculate start and end of the line using model coefficients
x_vals <- range(avg_outcome_annotated$mean_baseline, na.rm = TRUE)
# Convert xk_mean (mean_baseline) to treatment effect using model
y_vals <- beta_main + beta_across * x_vals

df_segment <- data.frame(
  x = x_vals[1], xend = x_vals[2],
  y = y_vals[1], yend = y_vals[2]
)
